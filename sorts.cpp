...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    // Using the insertion sort algorithm (pp. 332-333), sort s
    ...
    for (size_t k = 1; k < s.size(); k++) {
        ...
        while (l > 0  && comp(currentSensor, s[l-1])){ //while ((loc > 0) && (theArray[loc - 1] > nextItem))
            s[l] = s[l-1]; //theArray[loc] = theArray[loc - 1];
            l--; //loc--;
        }
         s[l] = currentSensor; //theArray[loc] = nextItem; // Insert nextItem into sorted region
